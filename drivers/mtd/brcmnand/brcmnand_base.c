 ...
 */
	...
	{	/* 6 Same old ID 0xD3, new part, so the old #define macro is kept, but IDstr is changed to reflect new part number */
		...
		.timing1 = 0, .timing2 = 0,
//		.nop=4,
		.nop=8,  // 4K pages of 8 512b sectors.. it seems the datasheet is wrong.. (tested with dm7020hd)
		.ctrlVersion = CONFIG_MTD_BRCMNAND_VERS_2_1,
	},
#endif
 ...
 */
uint32_t brcmnand_disable_read_ecc(struct brcmnand_chip *chip)
{
	uint32_t acc0;
	uint32_t acc;
#if CONFIG_MTD_BRCMNAND_VERSION >= CONFIG_MTD_BRCMNAND_VERS_3_3
...
printk("-->%s, page=%0llx\n", __FUNCTION__, page);}
	...
	if (likely(outp_oob == BRCMNAND_OOBBUF(chip->ctrl->buffers) || outp_buf == chip->ctrl->buffers->databuf))
		chip->pagebuf = -1LL;
...
printk("-->%s, offset=%0llx\n", __FUNCTION__, offset);}
	...
	if (unlikely(chip->pagebuf != page && outp_oob == BRCMNAND_OOBBUF(chip->ctrl->buffers)))
		chip->pagebuf = -1LL;
...
//nonBatch = 0;
...
#endif
	{
		while(1) {
			bytes = min(mtd->writesize - col, readlen);
			bufpoi = chip->ctrl->buffers->databuf;
...
printk("-->%s, offset=%0llx\n", __FUNCTION__, offset);}
	...
	if (likely(inp_oob == BRCMNAND_OOBBUF(chip->ctrl->buffers) || inp_buf == chip->ctrl->buffers->databuf || page == chip->pagebuf))
		chip->pagebuf = -1LL;
			   ...
			   const uint8_t* inp_oob, uint64_t page)
{
	...
	if (unlikely(inp_oob == BRCMNAND_OOBBUF(chip->ctrl->buffers) || page == chip->pagebuf))
		chip->pagebuf = -1LL;
...
static void handle_ecclevel_v0_1 (struct mtd_info *mtd, struct brcmnand_chip* chip, int cs)
{
	if (cs) {
...
#ifdef CONFIG_DREAMBOX_DM800
			/* We know that flash endAddr is 0x1800_0000 */
		...
		printk("Found NAND chip on Chip Select 0, size=%lldMB, base=%08x\n", mtd->size>>20, (unsigned int) chip->pbase);
...
int brcmnand_scan(struct mtd_info *mtd , int cs, int numchips )
{
	...
	volatile unsigned long __maybe_unused nand_select;
...
#if 0 && defined( CONFIG_BCM7401C0 ) || defined( CONFIG_BCM7118A0 )  || defined( CONFIG_BCM7403A0 )
...
#if 0//def CONFIG_MTD_PARTITIONS
