...
#define DMM_FETCH(name, fn, arg) do {				\
	const char *env = prom_getenv(name);			\
	if (env != NULL) {					\
		printk("Fetch var '%s' = '%s'\n", name, env);	\
		fn(env, arg);					\
	} else							\
		printk("Could not fetch var '%s'\n", name);	\
	} while(0)

	prom_init_cmdline();

	do {
		unsigned long low_mem_size = ((brcm_dram0_size_mb - 1) & 0xFF) + 1;
		unsigned long kernel_low_mem=0;
		uint32_t pm0DataLo, pm0DataHi;
		char *memp = strstr(arcs_cmdline, " mem=");

		if (memp) {
			char *ep;
			kernel_low_mem = simple_strtoul(memp+5, &ep, 0);
			ep = strchr(memp+1, ' ');
			if (ep)
				strcpy(memp, ep);
			else
		...
		if (kernel_low_mem > low_mem_size || kernel_low_mem == 0)
			/* fall back to standard behavior */
			DMM_FETCH("memsize", parse_ulong, &kernel_low_mem);

		BUG_ON(!kernel_low_mem || kernel_low_mem > low_mem_size);

		if (!strstr(arcs_cmdline, "bmem")) {
			unsigned long bmem = low_mem_size - kernel_low_mem;
			size_t l = strlen(arcs_cmdline);
			while(l > 0 && arcs_cmdline[l-1] == ' ')
				--l;
			sprintf(arcs_cmdline+l, " bmem=%luM@%luM", bmem, kernel_low_mem);
		}

		// read mac addr from bootloader
		pm0DataLo = readl((void*)0xB0080058);
		pm0DataHi = readl((void*)0xB008005c);
		brcm_primary_macaddr[0] = (pm0DataHi & 0x0000FF00) >> 8;
		brcm_primary_macaddr[1] = (pm0DataHi & 0x000000FF);
		brcm_primary_macaddr[2] = (pm0DataLo & 0xFF000000) >> 24;
...
static inline void __init brcm_wr_vec(unsigned long dst, char *start, char *end)
{
...
static inline void __init brcm_nmi_handler_setup(void)
{
...
unsigned long __init brcm_setup_ebase(void)
{
	unsigned long ebase = CKSEG0;
